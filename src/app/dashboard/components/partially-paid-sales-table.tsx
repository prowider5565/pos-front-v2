...
import type { PartiallyPaidSale } from "@/types/analytics"
import type { DateFilterParams } from "./dashboard-filter"
import { usePartiallyPaidSales } from "@/hooks/use-analytics"
...
interface PartiallyPaidSalesTableProps {
  filterParams?: DateFilterParams
}

const createColumns = (t: any): ColumnDef<PartiallyPaidSale>[] => [
  {
...
export function PartiallyPaidSalesTable({ filterParams }: PartiallyPaidSalesTableProps) {
  const { t } = useTranslation(["dashboard", "common"])
  const [page, setPage] = React.useState(1)
  const { data, isLoading, isError } = usePartiallyPaidSales(filterParams, page)
  ...
  if (isError) {
    return (
      <div className="text-center py-8 text-muted-foreground">
        Failed to load partially paid sales
      </div>
    )
  }
